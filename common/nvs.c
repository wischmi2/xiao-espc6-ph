                                           ...
                                           const char *defaultstr)
{
    ...
    {
        ESP_LOGW(TAG, "Value for key %s is too long for buffer (%zu bytes)", key, outsize);
    }
    else if (err != ESP_OK)
    {
...
bool nvs_write_str(const char *key, const char *str)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to set %s, err = %d", key, err);
        nvs_close(handle);
        return false;
    }
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to commit %s, err = %d", key, err);
        return false;
    }
    return true;
}
